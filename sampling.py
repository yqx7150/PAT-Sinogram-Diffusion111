...
class ReverseDiffusionPredictor(Predictor):
    ...
    def update_fn(self, x, t, good):

        f, G = self.rsde.discretize(x, t, good)
        z = torch.randn_like(x)
        x_mean = x - f
...
class LangevinCorrector(Corrector):
    ...
    def update_fn(self, x1, x2, x3, x_mean, t, good):
        sde = self.sde
        ...
        for i in range(n_steps):
            ...
            grad1 = score_fn(x1, t, good)  # 5
            grad2 = score_fn(x2, t, good)  # 5
            grad3 = score_fn(x3, t, good)  # 5
            ...
            x_mean = (
                x_mean + step_size[:, None, None, None] * (grad1 + grad2 + grad3) / 3.0
            )  # 7
...
def shared_predictor_update_fn(
    x, t, sde, model, predictor, probability_flow, continuous, good
):
    ...
    return predictor_obj.update_fn(x, t, good)


def shared_corrector_update_fn(
    x1, x2, x3, x_mean, t, sde, model, corrector, continuous, snr, n_steps, good
):
    ...
    return corrector_obj.update_fn(x1, x2, x3, x_mean, t, good)

...
def get_pc_inpainter(
    ...
    def pc_inpainter(model, dimg, data, good_input, unet_input, detector):
        ...
        with torch.no_grad():
            ...
            for i in range(detector):
                mask[(512 // detector) * i, :] = 1
                y0[(512 // detector) * i, :] = data2[i, :]
            ...
            for i in range(sde.N):
                ...
                # 保真
                x_mean = x_mean * (1 - mask) + y0
                x_show = x_mean.squeeze(0).squeeze(0)
                x_show = np.array(
                    ((x_show - x_show.min()) / (x_show.max() - x_show.min())).to("cpu")
                )
                cv2.imwrite(
                    "./temp_outcomes/testpadsecond_" + str(i) + ".png", x_show * 255.0
                )

...
import io
import likelihood
import controllable_generation_lzl_2 as controllable_generation
...
if sde.lower() == "vesde":
    for temp in range(38, 39):  # (24, 37) (37,51)   # (42, 46) (46,50)
        ...
        ckpt_filename = "/home/lqg/LJB/work_after_24_1_28/ncns+unt/exp_all_3_16/checkpoints/checkpoint_14.pth"
        print(ckpt_filename)
        ...
        for num in [4]:
            ...
            file_path = (
                "/home/lqg/LJB/work_after_24_1_28/Unet/data/test/xueguan/mat128/4.mat"
            )
            bad_input = np.array(loadmat(file_path)["sensor_data"], dtype=np.float32)
            # good_path = os.path.join(path_100, str(num) + ".png")
            good_path = (
                "/home/lqg/LJB/work_after_24_1_28/Unet/data/test/xueguan/png/4.png"
            )
            detector = 128
            good_input = cv2.imread(good_path, 0)
            good_input_nomarlized = good_input
            unet_input = get_unet_input(bad_input, length=512, low_detector=detector)
            ...
            x_result, psnr, ssim = pc_inpainter(
                ...
                dimg,
                bad_input,
                good_input_nomarlized,
                unet_input,
                detector,
            )  # 预测器校正器操作
            cv2.imwrite(
                "/home/lqg/LJB/work_after_24_1_28/diffu1/test_result_128_2000_32det_2024_3_1/"
                + str(temp)
                + "_"
                + str(num)
                + "_"

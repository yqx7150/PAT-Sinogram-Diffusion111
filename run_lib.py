...
def train(config, workdir):
    ...
    for step in range(
        initial_step, num_train_steps + 1
    ):  # ljb3: interation of training
        # print('initial_step, num_train_steps + 1 is :',initial_step, num_train_steps + 1)

        # Convert data to JAX arrays and normalize them. Use ._numpy() to avoid copy.
        # batch = torch.from_numpy(next(train_iter)['image']._numpy()).to(config.device).float()

        try:
            batch, goods = next(train_iter)
            batch = batch.float().cuda()
            goods = goods.float().cuda()
        ...
        # print("state,type(state) is :",state,type(state))   # ljb2: state is dict,contains the model and corresponded things

        loss = train_step_fn(state, batch, goods)
        ...
        if (
            ...
            if config.training.snapshot_sampling:
                ...
                sample, n = sampling_fn(score_model, goods)
                ...
                for num_bc in range(sample.shape[3]):
                    ...
                    fout = os.path.join(
                        this_sample_dir, "sample_" + str(num_bc) + ".png"
                    )

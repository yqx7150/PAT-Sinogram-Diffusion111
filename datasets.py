...
class GetMRI(Dataset):
    ...
    def __getitem__(self, index):
        siat_input = loadmat(self.data_names[index])["sensor_data"].astype(np.float32)
        unet_input = self.get_unet_input(siat_input, length=512, low_detector=128)
        # siat_input = self.padding_img(siat_input)
        # unet_input = self.padding_img(unet_input)
        return (siat_input[np.newaxis, :], unet_input[np.newaxis, :])
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
    ...
    dataset = GetMRI(
        root="/home/lqg/LJB/work_after_24_1_28/ncns+unt/data/train_18.5/mat_all_70/"
    )
    test_dataset = GetMRI(
        root="/home/lqg/LJB/work_after_24_1_28/ncns+unt/data/train_18.5/mat_all_70/"
    )
